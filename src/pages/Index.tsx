...
const Index = () => {
  ...
  const handleConfirmOccasion = useCallback(async (occasion: string) => {
    ...
    try {
      ...
      toast.error(error.message || "An unexpected error occurred during outfit analysis.");
    } finally {
  ...
  return (
    <div className="min-h-screen overflow-hidden paper-grain text-foreground">
      <SiteHeader
      ...
      <main>
        {activeView === "stylist" ? (
          <>
            ...
            />
            <AnalysisSection result={analysisResult} isLoading={isAnalyzing} />
            <RunwaySection />
          </>
        ) : (
          <HistoryDashboard
